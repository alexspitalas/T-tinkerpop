...
In addition to `PageRank.edges`, `PageRank.propertyName`, and `PageRank.times`, the `pageRank()`-step also accepts
`PageRank.startTime` and `PageRank.endTime` to restrict the computation to elements that are active within a temporal
window.
...
g.V().
  pageRank().
    ...
    with(PageRank.propertyName, 'rank2020').
  elementMap('name','rank2020')
----

The temporal options accept string values that are parsed as epoch-millisecond numbers or several common date/time
formats such as `yyyy-MM-dd` and ISO-8601 timestamps. `PageRank.endTime` requires `PageRank.startTime` to be
configured first. Temporal comparisons are inclusive at both boundaries when checking whether elements are active in
the window, but `PageRank.startTime` must still be strictly before `PageRank.endTime`.
